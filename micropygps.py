...
class MicropyGPS(object):
    ...
    #By Prajuab Riabroy to read the NMEA sentences from Valeport Tidemaster.
    def pvtma(self):
        """PVTMA: A NMEA style output is the standard data format and provides the most detailed information 
        from the instrument.  The TideMaster will always output the $PVTM0 string and will additionally 
        output a $PVTM1 or $PVTM2 string if an additional sensor is enabled. """
        self.tid_id = self.gps_segments[1]
        dt = self.gps_segments[2]
        self.tid_local_date = dt[6:8]+"/"+dt[4:6]+"/"+dt[0:4]
        tt = self.gps_segments[3]
        self.tid_local_time = tt[0:2]+":"+tt[2:4]+":"+tt[4:6]
        self.tid_height = self.gps_segments[4]
        self.tid_height_stdev = self.gps_segments[5]
        self.tid_height_units = self.gps_segments[6]
        self.tid_int_battery_voltage = self.gps_segments[7]
        self.tid_ext_battery_voltage = self.gps_segments[8]  


        return True

    #By Prajuab Riabroy to read the SDDPT NMEA sentences from Echo sounder.
    def sddpt(self):
        """SDDPT: """
        self.nmea_sentence = 'SDDPT'
        self.depth_meter = self.gps_segments[1]
        self.depth_unit = 'M'
        return True

    #By Prajuab Riabroy to read the SDDBT NMEA sentences from Echo sounder.
    def sddbt(self):
        """SDDBT: $SDDBT,1.8,f,0.5,M,0.3,F*09"""
        self.nmea_sentence = 'SDDBT'
        self.depth_feet = self.gps_segments[1]
        self.depth_unit1 = self.gps_segments[2]
        self.depth_meter = self.gps_segments[3]
        self.depth_unit2 = self.gps_segments[4]
        self.depth_fathom = self.gps_segments[5]
        self.depth_unit3 = self.gps_segments[6]
        return True
                    ...
                    if not self.process_crc:
                        ...
                        if len(self.gps_segments[self.active_segment]) == 2:
                            try:
                                ...
                                #print(f"Compare checksum from xor: {self.crc_xor} and from nmea sentence: {final_crc}")
    ...
    supported_sentences = {'GPRMC': gprmc, 'GLRMC': gprmc,
                           ...
                           'GNGSA': gpgsa, 'PVTMA': pvtma,
                           'SDDPT': sddpt, 'SDDBT': sddbt,
                          }
